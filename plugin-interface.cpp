...
#define CASE(kind) case PluginInterface::Kind##kind: os << #kind; break;
	...
	CASE(Tanh) CASE(Logistic) CASE(HardSwish) CASE(Sub) CASE(Mul) CASE(Div) CASE(Maximum) CASE(Minimum) CASE(Transpose) CASE(Reshape) CASE(Softmax) CASE(Concatenation)
	CASE(StridedSlice) CASE(Mean) CASE(ArgMax) CASE(ArgMin) CASE(SquaredDifference) CASE(ResizeBilinear)
	CASE(Unknown)
	}
#undef CASE
	return os;
